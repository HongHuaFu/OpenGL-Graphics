...
unsigned int geo__cubeVAO = 0;
unsigned int geo__cubeVBO = 0;
void Geometry::renderCube()
{
    if (geo__cubeVAO == 0)
    {
        ...
        glGenVertexArrays(1, &geo__cubeVAO);
        glGenBuffers(1, &geo__cubeVBO);
        glBindBuffer(GL_ARRAY_BUFFER, geo__cubeVBO);
        glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
        glBindVertexArray(geo__cubeVAO);
        glEnableVertexAttribArray(0);
...
unsigned int renderSphere_sphereVAO = 0;
unsigned int renderSphere_indexCount;
void Geometry::renderSphere()
{
	if (renderSphere_sphereVAO == 0)
	{
		glGenVertexArrays(1, &renderSphere_sphereVAO);
		...
		renderSphere_indexCount = indices.size();
		...
		glBindVertexArray(renderSphere_sphereVAO);
	...
	glBindVertexArray(renderSphere_sphereVAO);
	glDrawElements(GL_TRIANGLE_STRIP, renderSphere_indexCount, GL_UNSIGNED_INT, 0);
}
